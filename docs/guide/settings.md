...
`Show break on` chooses where overlays appear: `All monitors` (the default), `Primary monitor`, or `Monitor under cursor` (the display the mouse cursor is on at break-fire time). If active-monitor detection fails for any reason, Entracte falls back to the primary monitor.

`All monitors` is the default because a break you can sidestep by glancing at a second screen isn't really a break. If you'd rather keep a secondary display clear — for reference material, a dashboard, or a machine you're watching — switch to `Primary monitor` or `Monitor under cursor`. On a single-display setup all three behave identically.
...
- **Manual pause** — pause until a specific date and time (handy over a holiday when you're at the computer but not working), and a Resume button showing the current pause state. The same picker is reachable straight from the tray: **Pause for… → Pause until…** opens a small popup. The popup orders its date fields by your OS region and shows the time in your **System → Display** 12h/24h format, so it matches the rest of the app rather than the WebView's default locale.
