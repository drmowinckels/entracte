...
2. **Sandbox escape.** _Mitigation:_ use a mature, widely-deployed runtime (wasmtime, via extism), keep it patched, ship no `unsafe` host glue beyond what the runtime requires, and expose the smallest possible host-function surface. Residual risk is the runtime's own; tracked with dependency updates.
   ...
   Note that "WASI off" also narrows which of the runtime's own advisories apply to us. A wasmtime advisory scoped to the WASI filesystem surface — as RUSTSEC-2026-0269 is ([#323](https://github.com/drmowinckels/entracte/issues/323)) — is not reachable from this configuration: with no WASI in the linker there are no preopened directories, and a module importing `wasi_snapshot_preview1` fails to link rather than running. This is not a comment-only claim; `rejects_a_module_importing_the_wasi_filesystem` in `plugins/runtime.rs` fails if WASI is ever switched on. Advisories in wasmtime's **core** (compiler, `Store`, instance allocation) are not narrowed this way and must still be assessed on their own terms.
