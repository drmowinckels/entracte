...
Each hook stores a single command line, parsed with POSIX-style argv splitting (via `shell-words`). The first token is the program, the rest are arguments — no shell is involved, so pipes, redirects, globs, and environment expansion do not work directly. If you want those, write your command as `sh -c "..."` (or `cmd /C "..."` on Windows) and quote the script accordingly. Each hook runs on its own background thread; Entracte does not capture its output or restart it on failure, and spawn failures are logged to stderr. A hook is given **up to 30 seconds** to finish — long enough for the quick notify/log commands hooks are meant for — after which it is killed, so a command that hangs (an infinite loop, or one waiting on input) cannot run forever. Keep hooks short; if you need long-running work, have the hook kick it off and return (e.g. `sh -c "your-long-task &"`).
