...
- **Preferences window controls now have selectable Wayland workarounds.** The 0.0.6 fix for unclickable close/minimise buttons on GNOME/Wayland (a 1px resize nudge) was a no-op on some compositors: the resize and its restore ran in the same event-loop turn, so Wayland coalesced them and never emitted the `configure` the workaround relied on. The nudge now defers its restore onto a later tick so the size change genuinely commits, and the `ENTRACTE_WL_FIX` environment variable selects the strategy — `nudge` (default), `maximize` (programmatic maximize/restore, mirroring the manual double-click that clears it), `titlebar` (drop the custom GTK headerbar), or `off`. The active strategy is recorded in the startup banner so diagnostics reports show which one ran. ([#139](https://github.com/drmowinckels/entracte/issues/139))
