...
- **Patch coverage target is 100%.** Codecov fails the PR if any new line is uncovered. First instinct: restructure so the line is reachable from a test (the `*_impl` / pure-function pattern). When a line is genuinely untestable — a `tauri::command` body that is a thin shim, an async-spawn loop call site, etc. — annotate it with `// LCOV_EXCL_LINE` (single line) or `// LCOV_EXCL_START` / `// LCOV_EXCL_STOP` (block) and add a one-line comment naming why. Don't lower the codecov target to ease a PR through; deal with the exception at the source.
