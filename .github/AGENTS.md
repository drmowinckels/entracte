...
- **In-app keyboard shortcuts get `aria-keyshortcuts`; global-hotkey defaults avoid the VoiceOver modifier.** Surface any in-app shortcut on the control it activates via `aria-keyshortcuts` (e.g. the overlay's Skip carries `aria-keyshortcuts="Escape"`). The System-tab global hotkeys are OS-level and off by default — Entracte ships no pre-bound chord. If you ever add a default or example accelerator, **don't use `Ctrl+Option`**: that's VoiceOver's default modifier ("VO keys") on macOS and the chord would be swallowed before it reached the app. The example placeholder `CmdOrCtrl+Alt+P` resolves to `Cmd+Option` on macOS and is safe.
...
- **Patch coverage target is 100%.** Codecov fails the PR if any new line is uncovered. First instinct: restructure so the line is reachable from a test (the `*_impl` / pure-function pattern) — e.g. the per-OS detectors split their parse step into a pure, un-gated helper that's unit-tested on every OS, leaving only the thin "shell out to the OS command and hand its stdout to the parser" call site uncovered. **Note:** `cargo-llvm-cov` (our coverage tool) does **not** honor lcov's `LCOV_EXCL_LINE` / `LCOV_EXCL_START` comments — that's a geninfo feature. To genuinely exclude a function, use `#[cfg_attr(coverage_nightly, coverage(off))]` (requires the crate-root `#![cfg_attr(coverage_nightly, feature(coverage_attribute))]` and running coverage on nightly). For the handful of per-OS call sites that can't run in headless CI (no live `systemd-inhibit`/`gsettings`/`gdbus`/`pmset`/`powercfg`/registry), the current practice is an **admin bypass** of `codecov/patch` on that PR — the lines are unreachable in CI by construction, which is exactly the sanctioned bypass case. Don't lower the codecov target to ease a PR through.
